//! Loads [`Config`] from built-in defaults, a TOML file, then `UWUU_*`
...
pub const DEFAULT_PATH: &str = "uwuubooru.toml";

/// Environment variables map to keys with `__` as the section separator:
/// `UWUU_DATABASE__URL` sets `database.url`.
const ENV_PREFIX: &str = "UWUU_";
...
pub fn load(path: Option<&Path>) -> anyhow::Result<Config> {
    ...
    let config: Config = figment
        // UWUU_CONFIG selects the file itself and is not a config key.
...
mod tests {
    ...
    fn env_overrides_file_overrides_defaults() {
        Jail::expect_with(|jail| {
            jail.create_file(
                ...
                r#"
                [database]
                url = "postgres://file/uwuu"
                max_connections = 4
                "#,
            )?;
            jail.set_env("UWUU_DATABASE__URL", "postgres://env/uwuu");
            jail.set_env("UWUU_CONFIG", "ignored.toml");

            let config = load_in_jail(None)?;
            assert_eq!(config.database.url, "postgres://env/uwuu");
    ...
    fn env_alone_is_enough() {
        Jail::expect_with(|jail| {
            jail.set_env("UWUU_DATABASE__URL", "postgres://env/uwuu");
            jail.set_env(
                "UWUU_DATABASE__REPLICAS",
                r#"["postgres://r1/uwuu", "postgres://r2/uwuu"]"#,
            );
            jail.set_env("UWUU_TELEMETRY__LOG_FORMAT", "json");

            let config = load_in_jail(None)?;
            assert_eq!(config.database.replicas.len(), 2);
            assert_eq!(
                config.telemetry.log_format,
                uwuu_core::config::LogFormat::Json
            );
            Ok(())
        });
    }
    ...
    fn rejects_unknown_keys() {
        Jail::expect_with(|jail| {
            jail.set_env("UWUU_DATABASE__URL", "postgres://env/uwuu");
            jail.set_env("UWUU_DATABSE__URL", "typo");
            let err = load_in_jail(None).unwrap_err();
