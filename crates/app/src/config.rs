//! Loads [`Config`] from built-in defaults, a TOML file, then `MOEKURA_*`
...
mod tests {
    ...
    fn env_overrides_file_overrides_defaults() {
        Jail::expect_with(|jail| {
            jail.create_file(
                ...
                r#"
                [database]
                url = "postgres://file/moekura"
                max_connections = 4
                "#,
            )?;
            jail.set_env("MOEKURA_DATABASE__URL", "postgres://env/moekura");
            jail.set_env("MOEKURA_CONFIG", "ignored.toml");

            let config = load_in_jail(None)?;
            assert_eq!(config.database.url, "postgres://env/moekura");
    ...
    fn env_alone_is_enough() {
        Jail::expect_with(|jail| {
            jail.set_env("MOEKURA_DATABASE__URL", "postgres://env/moekura");
            jail.set_env(
                "MOEKURA_DATABASE__REPLICAS",
                r#"["postgres://r1/moekura", "postgres://r2/moekura"]"#,
            );
            jail.set_env("MOEKURA_TELEMETRY__LOG_FORMAT", "json");

            let config = load_in_jail(None)?;
            assert_eq!(config.database.replicas.len(), 2);
            assert_eq!(
                config.telemetry.log_format,
                moekura_core::config::LogFormat::Json
            );
            Ok(())
        });
    }
    ...
    fn rejects_unknown_keys() {
        Jail::expect_with(|jail| {
            jail.set_env("MOEKURA_DATABASE__URL", "postgres://env/moekura");
            jail.set_env("MOEKURA_DATABSE__URL", "typo");
            let err = load_in_jail(None).unwrap_err();
